...
@section('content')
    ...
    <i>Resetting a password sets it to be the student id with 5 0's appended to it. e.g. JDoe00000</i>
    <div class="card mt-3">
        <div class=" col-md-12 table card-body task-table">
            ...
            @foreach ($students as $student)
                <div class="row task-row d-flex justify-content-between pt-1 pb-1">
                    <div class="col-md-3 task-title pt-2">{{ $student->student_id }}</div>
                    <div class="col-md-3 reset-password">
                        <input type="hidden" class="reset-pass-id" name="id" value="{{$student->id}}">
                        <a class="btn btn-sm btn-primary reset-password-btn">Reset password</a>
                    </div>
                    <div class="col-md-3 delete-user">
                        <input type="hidden" class="delete-user-id" name="id" value="{{$student->id}}">
    ...
    <script>
        $('.reset-password-btn').click(function(){
            ...
            let $data = {user: 'student', id: $(this).closest('.reset-password').find('.reset-pass-id').val()},
                $button = $(this);
            $.ajax({
                url: '/resetPassword',
                data: $data,
